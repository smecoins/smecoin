...
#if defined(NDEBUG)
# error "Smecoins cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Smecoins Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Smecoins: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Smecoins: find block wanted by given orphan block
...
// Smecoins: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Smecoins miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Smecoins: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Smecoins: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Smecoins: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Smecoins: track money supply and mint amount info
...
// Smecoins: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Smecoins: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Smecoins: compute stake entropy bit for stake modifier
    ...
    // Smecoins: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Smecoins: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Smecoins: attempt to generate suitable proof-of-stake
